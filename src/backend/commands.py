...
class Commands:
    ...
    async def handle_cmd(self, websocket: WebSocket,
    ...
    @validate_arguments(config=pydantic_config)
    async def cmd_help(self, websocket: WebSocket,
                       cmd_name: Optional[str] = None):
        if not cmd_name:
            all_fct = inspect.getmembers(
                Commands, predicate=inspect.isfunction)
            fct_names = ', '.join(x[0][4:]
                                  for x in all_fct if x[0].startswith('cmd_'))
